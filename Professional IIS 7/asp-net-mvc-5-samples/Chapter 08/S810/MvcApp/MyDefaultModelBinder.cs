...
namespace MvcApp
{
    public class MyDefaultModelBinder : IModelBinder
    {
        ...
        private void BindProperty(ControllerContext controllerContext, ModelBindingContext bindingContext, PropertyDescriptor propertyDescriptor)
        {
            ...
            this.ValidateRequiredPropertyValue(controllerContext, bindingContext, metadata, propertyValue);
            propertyDescriptor.SetValue(bindingContext.Model, propertyValue);
        }

        private void ValidateRequiredPropertyValue(ControllerContext controllerContext, ModelBindingContext bindingContext, ModelMetadata propertyModelMetadata, object propertyValue)
        {
            string key = (bindingContext.ModelName ?? "") + "." + (propertyModelMetadata.PropertyName ?? "");
            key = key.Trim('.');
            ModelStateDictionary modelState = bindingContext.ModelState;
            if (null == propertyValue)
            {
                //利用注册ModelValidatorProvider提供的ModelValidator实施验证
                ModelValidator validator = ModelValidatorProviders.Providers.GetValidators(propertyModelMetadata, controllerContext).FirstOrDefault(v => v.IsRequired);
                if (null != validator)
                {
                ...
                //针对非可空值类型
                Type propertyType = propertyModelMetadata.ModelType;
                if (propertyType.IsValueType && Nullable.GetUnderlyingType(propertyType) == null && modelState.IsValidField(key))
                {
                    modelState.AddModelError(key, "A value is required.");
                }
            }
        }
